  ...
  return (
    <div className="space-y-6">
      ...
      {visibleSections.roadmap && (() => {
        ...
        if (capexProducts.length === 0) return null;
        ...
        return (
          <SectionCard title="Plan d'Investissement CAPEX — Détail par Produit" id="capex-detail">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {productTimelines.map(({ product, payments, totalScheduled, scheduledPercent }) => (
                    ...
                    {payments.length > 0 ? (
                      <div className="px-4 py-3">
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {payments.sort((a, b) => a.year - b.year || a.month - b.month).map(p => (
                        ...
                        <div className="mt-2 h-2 bg-muted rounded-full overflow-hidden">
                          <div className="h-full bg-primary rounded-full transition-all" style={{ width: `${Math.min(100, scheduledPercent)}%` }} />
                        </div>
                      </div>
                    ) : (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-muted/30">
                        ...
                        {years.map(y => <th key={y} className="text-right py-2 px-3 font-semibold">{y}</th>)}
                    ...
                    <tbody>
                      {productTimelines.map(({ product, payments }) => (
                        <tr key={product.id} className="border-b">
                          ...
                          {years.map(year => {
                            const yp = payments.filter(p => p.year === year);
                            const amount = yp.length > 0 ? yp.reduce((s, p) => s + p.amount, 0) : (payments.length === 0 && product.launchYear === year ? product.devCost : 0);
                            return (
                              <td key={year} className={cn("py-2 px-3 text-right font-mono-numbers", amount > 0 ? "font-medium" : "text-muted-foreground")}>
                                {amount > 0 ? formatCurrency(amount, true) : '—'}
                              </td>
                            );
                          })}
                          <td className="py-2 px-3 text-right font-mono-numbers font-semibold">{formatCurrency(product.devCost, true)}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
                      <tr className="border-t-2 bg-muted/30 font-semibold">
                        ...
                        {capexByYear.map(({ year, total }) => (
                          <td key={year} className="py-2 px-3 text-right font-mono-numbers">{total > 0 ? formatCurrency(total, true) : '—'}</td>
                        ))}
                        <td className="py-2 px-3 text-right font-mono-numbers text-primary">{formatCurrency(grandTotal, true)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
