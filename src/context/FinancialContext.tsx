...
export function FinancialProvider({ children }: { children: ReactNode }) {
  ...
  const computed = useMemo<ComputedValues>(() => {
    ...
    // Merge funding plan entries as FundingRounds so they appear in fundingInjection & totalFundingRaised
    const fundingPlan = state.fundingPlan;
    let allFundingRounds = [...roundsWithQuarter];
    if (fundingPlan?.enabled && fundingPlan.entries.length > 0) {
      fundingPlan.entries.forEach(entry => {
        Object.entries(entry.amountsByYear).forEach(([yearStr, amount]) => {
          if (amount > 0) {
            allFundingRounds.push({
              id: `fp-${entry.id}-${yearStr}`,
              name: entry.label,
              amount,
              preMoneyValuation: 0,
              year: Number(yearStr),
    ...
    const effectiveAllRounds = excludeFundingFromTreasury ? [] : allFundingRounds;
